...
const findForexSimbols = (forexName) => {
    let simbolsArray = [];

    for(let key in physicalCurrencyList){
        if((physicalCurrencyList[key]['currency code']).toString().toLowerCase().includes(simbol.toLowerCase())){
            let forexObj = {
                forexCode : physicalCurrencyList[key]['currency code'],
                forexName: physicalCurrencyList[key]['currency name']
            }
            simbolsArray.push(forexObj);
            continue;
        }

        if((physicalCurrencyList[key]['currency name']).toString().toLowerCase().includes(simbol.toLowerCase())){
            let forexObj = {
                forexCode : physicalCurrencyList[key]['currency code'],
                forexName: physicalCurrencyList[key]['currency name']
            }
...
let array = findForexSimbols(simbol);
